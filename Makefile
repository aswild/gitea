...
.PHONY: clean-all
clean-all: clean
	rm -rf $(WEBPACK_DEST_ENTRIES) $(FOMANTIC_DEST_DIR) $(MAKE_EVIDENCE_DIR)
.PHONY: distclean
distclean: clean-all
...
watch-frontend: $(FOMANTIC_DEST) node_modules
...
.PHONY: docker
docker:
	docker build $(DOCKER_NO_CACHE) --build-arg VERSION='$(GITEA_VERSION)' -t $(DOCKER_REF) .
